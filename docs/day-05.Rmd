...
filter(covid, date == max_date) %>%
  group_by(state) %>%
  summarize(StateTot = sum(cases, na.rm = TRUE),
              n = n()) %>%
  arrange(-StateTot) %>% 
  head(5)
```
...
max_date = max(covid$date, na.rm = TRUE)
filter(covid, date == max_date) %>%
  mutate(deathRatio = deaths/cases * 100) %>%
  arrange(-deathRatio) %>%
  head(5)
```
...
filter(covid, date == max_date) %>%
  group_by(state) %>%
  summarize(StateTotCase = sum(cases, na.rm = TRUE), StateTotDeath = sum(deaths, na.rm = TRUE),
            n = n()) %>%
  mutate(deathRatio = StateTotDeath/StateTotCase * 100) %>%
  arrange(-deathRatio) %>%
  head(5)
```
